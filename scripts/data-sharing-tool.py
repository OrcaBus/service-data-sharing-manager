...
def list_packages(package_name: Optional[str]) -> List[PackageRequestResponseDict]:
    ...
    response_results = sorted(
        response.json()['results'],
        key=lambda package_obj_iter_: package_obj_iter_['requestTime'],
        reverse=True
    )
...
def generate_package(
    ...
    library_ids = list(map(
        lambda library_id_iter_: re.sub(r"_(?:topup|rerun)(?:\d+)?$", "", library_id_iter_),
...
def _dispatch():
    ...
    try:
        ...
        raise ValueError("You are not logged into AWS") from e
    if not account_id == AWS_PRODUCTION_ACCOUNT_ID:
        logger.warning(
            f"Warning, you are not using the production account. "
            f"You are using {account_id}. "
            f"Expected account is {AWS_PRODUCTION_ACCOUNT_ID}"
        )
