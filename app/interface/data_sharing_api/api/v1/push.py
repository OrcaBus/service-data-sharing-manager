...
@router.patch(
    ...
    description=dedent("""
    This updates a job status. This will update a job and set the status to the new status.
...
async def update_job(
    ...
    if push_job_status_obj.status not in ['RUNNING', 'FAILED', 'SUCCEEDED']:
    ...
    try:
        ...
        if push_job_obj.status in ['FAILED', 'SUCCEEDED']:
...
async def abort_job(push_job_id: str = Depends(sanitise_psh_orcabus_id)) -> PushJobResponse:
    try:
        ...
        if not push_job_obj.status in ['PENDING', 'RUNNING']:
            raise AssertionError("Job is not in a state that can be aborted")

        push_job_obj.status = 'ABORTED'
